...
const ExamStudentsControl = React.memo(({data}: {data: any}) => {
  ...
  const { data: data1, isFetching, refetch: refetch2, isLoading } = useGetAllData({
    queryKey: ['final-exam-tests', searchFName, searchMName, searchLName, urlValue?.filter?.group_id, urlValue.perPage, urlValue.currentPage],
    ...
    let arr: any = [
      {
        title: "№",
        dataIndex: "order",
        render: (_: any, __: any, i: number) => number_order(urlValue.currentPage, urlValue.perPage, Number(i), isLoading),
        width: 50,
      },
      {
        title: "F.I.O",
  ...
  const columnsForInside: ColumnsType<any> = React.useMemo(() => {
    ...
    let arr: any = [
      ...
      checkPermission("final-exam-test-start_get") ? {
        ...
        render: (e: any) => <Tooltip placement="topLeft" title={t("View")}><Eye16Filled className="view cursor-pointer" onClick={() => {setIsTestViewOpen(true); setselectedTestId(e?.id)}} /></Tooltip>
      } : undefined,
      checkPermission("final-exam-test-start_update") ? {
        title: t("Actions"),
        align: "center",
        render: (e: any) => (
            // (e?.status !== 3 && (data?.status == 2 || data?.status == 3)) ? 
  ...
  const exportExcel = async () => {
    ...
    setLoading(false);
    excelExport(arr, `${data?.eduSemestrSubject?.subject?.name} fanidan oraliq nazorat`);
  }

  ...
  ...( is_subject ? [
      {
        ...
        url: `kafedras`,
        required: true,
        span: { xs: 24, sm: 24, md: 12, lg: 16, xl: 8 },
        render: (e:any) => e?.name,
...
const FormExamTestQuestionUI = ({ data, refetch, setisEdit, isEdit, isSubject }: { data: ITestQuestion | undefined; refetch: IRefetch, setisEdit: Dispatch<boolean>, isEdit: boolean, isSubject?: boolean }) => {
  ...
  const { mutate, isLoading } = useMutation({
    mutationFn: (newVals: any) => submitExamTest(test_id, {subject_semestr_id, ...newVals, upload: fileList[0]?.originFileObj, type: 1 }),
    onSuccess: async (res) => {
      setisEdit(false)
      Notification("success", (test_id && test_id !== "0") ? "update" : "create", res?.message)
      isSubject ? navigate(`/tests/view/${res?.data?.id}`) : navigate(`/subject/${subject_semestr_id}/${res?.data?.subject_id}/exam-tests/update/${res?.data?.id}`)

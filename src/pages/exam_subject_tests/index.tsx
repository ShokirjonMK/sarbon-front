...
const SubjectExamTest: React.FC = (): JSX.Element => {
  ...
  const { data, isLoading, refetch } = useGetAllData<ITestQuestion>({
    queryKey: ["tests", id, urlValue.perPage, urlValue.currentPage, urlValue.filter?.language_id, urlValue.filter?.exam_type_id],
    urlParams: { "per-page": urlValue.perPage, page: urlValue.currentPage, filter: { subject_semestr_id: id, type: 2, language_id: urlValue.filter?.language_id, exam_type_id: urlValue.filter?.exam_type_id } },
  ...
  const { mutate: importExamTest } = useMutation({
    mutationFn: ({ file, exam_type_id, language_id }: { file: any, exam_type_id?: number, language_id?: number }) => importExamTestToExcel(id, file, exam_type_id, language_id),
    onSuccess: async (res) => {
      refetch();
  ...
  const { mutate: deleteAllMutation, isLoading: deleteloadingAllTest } = useMutation({
    mutationFn: async () => {
      ...
      formdata.append("language_id", String(selectedLangId));
    ...
    <Spin spinning={isLoading} size="small">
        ...
        <div className="flex justify-end items-center mb-3 gap-3">
...

          {
            checkPermission("test_create") ? <>
              ...
              <CreateBtn
                onClick={() => navigate(`/subject/${id}/${subject_id}/exam-tests/update/0`)}
          ...
          data?.items?.map((item, index) => (
            <div key={item?.id} className='bg-zinc-50 p-3 rounded-md mb-5 hover:shadow-lg relative test-list-actions-wrapper'>
              <div className='d-f absolute top-3 right-3 p-1 bg-zinc-50 test-list-actions'>
                ...
                {checkPermission("test_view") ? <Button onClick={() => navigate(`/subject/${id}/${item?.subject_id}/exam-tests/update/${item?.id}`)} ><Eye20Filled className="view" /></Button> : ""}
              </div>
      ...
      <Modal title={t("Exam test")} open={isModalOpen} onOk={() => setIsModalOpen(false)} onCancel={() => setIsModalOpen(false)} footer={false}>
        <Form
          form={form}
          layout="vertical"
          initialValues={{language_id: 1}}
          onFinish={(vals: any) => importExamTest({ exam_type_id: vals?.exam_type_id, language_id: vals?.language_id, file: test_file })}
          className='my-4'
        >
          <Form.Item
            name={`exam_type_id`}

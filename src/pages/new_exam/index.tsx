...
const NewExam: React.FC = (): JSX.Element => {
  ...
  const { data, refetch, isLoading } = useGetAllData<IFinalExam>({
    queryKey: ["final-exams", urlValue.perPage, urlValue.currentPage, urlValue?.filter, urlValue.filter_like, {exams_type_id: 3}],
    ...
    urlParams: { 
      ...
      filter: JSON.stringify({...urlValue?.filter, vedomst: urlValue.filter_like.vedomst, status: urlValue.filter.status_id, exams_type_id: 3}) 
    },
    options: {
  ...
  const columns: ColumnsType<IFinalExam> = React.useMemo(() => [
    ...
    },
    {
      title: t('Date'),
      width: 110,
      render: (e) => dateParserToDatePicker(e?.start_time) + " / " + dateParserToDatePicker(e?.finish_time)
    },
    // {
    //   title: t('Para'),
    //   dataIndex: 'para',
  ...
  useBreadCrumb({pageTitle: t("Final exam control"), breadcrumb: [
    {name: "Home", path: '/'},
    {name: t("Final exam control"), path: '/exams'},
  ]})
